...
1. Overall.class: методы insertClipBoardToBDValueList, get_2nd_ListSizeAs_1st_ListSize, getCollectionFromClipBoard, getClipboardText. Добавлен механизм, с помощью которого можно копировать из журнала градуировок ячейки с показаниями конкретного прибора, после чего, при нажатии на форме соответствующей кнопки, сохраненная строка в буфере обмена преобразуется в массив данных и заносится в соответствующие ячейки формы  
    ...
    б) Не обязательно копировать количество ячеек строго совпадающее с количеством полей в форме. Алгоритм автоматически преобразует кол-во скопированных данных. Данные заносятся снизу в вверх (на мелкие точки не всегда есть данные в журнале, поэтому если их нет, их можно и не копировать)     
...
3. Добавлен механизм загрузки параметров для макроса (файл "settings.cfg"), параметры храняться как ключ/значение, поэтому нет разницы сколько параметров нужно для конкретной версии или в каком порядке/количестве параметры сохранены в этом файле. Макрос парсит параметры и заберает те, что нужны
* <b>2.90</b> - 11/09/20
...
1. Для БДКГ23/22, 1103/1121/1123, 224, 6130, 2503, 3509 разделмтель для список номеров -- ";"
